...
__export(index_exports, {
  ...
  bankrPayer: () => bankrPayer,
  callPaidEndpoint: () => callPaidEndpoint,
...
function useX402Payment(config) {
  ...
  const [pricePaid, setPricePaid] = (0, import_react.useState)(null);
  const configRef = (0, import_react.useRef)(config);
  configRef.current = config;
  const pay = (0, import_react.useCallback)(async () => {
    setLoading(true);
    setError(null);
    try {
...
function X402Paywall(props) {
  ...
  const memoConfig = (0, import_react2.useMemo)(
    () => config,
  ...
  return (0, import_react2.createElement)(DefaultPaywallUI, {
    state,
...
function DefaultPaywallUI(props) {
  ...
  rows.push(
    (0, import_react2.createElement)(PayButton, { key: "btn", state })
  );
  if (state.error) {
    rows.push(
      (0, import_react2.createElement)(
